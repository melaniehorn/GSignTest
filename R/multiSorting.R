...
#'
#' @param data [\code{data.frame}]\cr \code{data.frame} where each row consists of one numeric observation.
#' @param method [\code{character(1)}]\cr Method for sorting, see Details.
#' @param control Further Arguments for the methods, see Details.
#'
...
multiSorting <- function(data, method = "identity", control = list()) {
  ...
  assert_choice(method, choices = c("identity", "random", "norm", "median",
    "dimSort", "weightedSum", "projection", "nonDomSort", "clust", "nn", "shp"))
